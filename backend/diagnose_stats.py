...
def run_test():
    ...
    queries = {
        "Total Active Openings": f"SELECT COUNT(*) FROM joborder WHERE (status = 'Active' OR status = '100') {date_filter_job.replace('WHERE', 'AND') if date_filter_job else ''}",
        "Enrolled Candidates": f"""
            ...
            {date_filter_cj_created.replace('WHERE', 'AND') if date_filter_cj_created else ''}
        """,
        "Total Candidates": f"SELECT COUNT(*) FROM candidate {date_filter_cand}",
    ...
    with engine.connect() as conn:
        ...
        for name, sql in queries.items():
            ...
            try:
                ...
                if "SELECT COUNT" in sql or "SELECT AVG" in sql:
