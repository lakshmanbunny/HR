...
@router.get("/stats")
async def get_stats(days: int = 30, job_status: Optional[str] = None, joborder_id: Optional[int] = None, recruiter_id: Optional[int] = None, start_date: Optional[str] = None, end_date: Optional[str] = None, db: Session = Depends(get_db)):
    """
    Returns real-time recruitment metrics from the production database.
    """
    try:
        stats = repository.get_recruitment_stats(db, days=days, job_status=job_status, job_id=joborder_id, recruiter_id=recruiter_id, start_date=start_date, end_date=end_date)
        return stats
    except Exception as e:
        import traceback
        traceback.print_exc()
